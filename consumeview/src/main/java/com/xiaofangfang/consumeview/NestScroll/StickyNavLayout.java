...
public class StickyNavLayout extends LinearLayout implements NestedScrollingParent {
    ...
    /**
     * 以startNestedScroll方法为例。当子view开始滑动时，会调用startNestedScroll方法，在该方法里面，主要实现就是获取当前的view的父view,
     * 如果父view实现了NestedScrollingParent 接口，就会回调onStartNestedScroll。然后在这些回调实现一些界面的变动，就有联动的效果了。
     */
    @Override
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        boolean hiddenTop = dy > 0 && getScrollY() < mTopViewHeight;//dy代表的是按下的rowY-moveRawY
        boolean showTop = dy < 0 && getScrollY() >= 0 && !ViewCompat.canScrollVertically(target, -1);

        if (hiddenTop || showTop) {//当上面的条件满足时,就去滚动自身,
            scrollBy(0, dy);
            consumed[1] = dy;//传递的是我们需要拦截的滚动距离
        }
    }

...
public class CustomProcessor extends AbstractProcessor {
     ...
     * @param roundEnv
     * @return
     * 如果返回 true，则这些注解已声明并且不要求后续 Processor 处理它们；如果返回 false，则这些注解未声明并且可能要求后续 Processor 处理它们。
     */
    @Override
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        try {

            String resultPath = processingEnv.getOptions().get(CUSTOM_ANNOTATION);//io.github.yuweiguocn.MyCustomAnnotation
            if (resultPath == null) {
            ...
            if (roundEnv.processingOver()) {//如果循环处理完成返回true，否则返回false。
            ...
            for (Element element : roundEnv.getElementsAnnotatedWith(CustomAnnotation.class)) {
                ...
                ExecutableElement execElement = (ExecutableElement) element;//因为是在方法上面使用的，所以是一个ExecutableElement
                TypeElement classElement = (TypeElement) execElement.getEnclosingElement();
                result.add(classElement.getSimpleName().toString() + "#" + execElement.getSimpleName().toString());//获得类的名称#方法名称
            }
            if (!result.isEmpty()) {
    ...
    private void generateFile(String path) {
        ...
        try {
            JavaFileObject sourceFile = filer.createSourceFile(path);//创建源代码文件
            int period = path.lastIndexOf('.');
            String myPackage = period > 0 ? path.substring(0, period) : null;//获得包的名称
            String clazz = path.substring(period + 1);//获得类名
